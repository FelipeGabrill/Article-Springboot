...
public class CongressoSpec {
    ...
    public static Specification<JpaCongressoEntity> nameContains(String name) {
    ...
    public static Specification<JpaCongressoEntity> knowledgeAreaContains(String knowledgeArea) {
        return (root, query, cb) -> {
            if (ObjectUtils.isEmpty(knowledgeArea)) return null;
            SetJoin<JpaCongressoEntity, String> join = root.joinSet("knowledgeArea");
            query.distinct(true);
            return cb.like(cb.lower(join), "%" + knowledgeArea.toLowerCase() + "%");
        };
    }

    public static Specification<JpaCongressoEntity> hasCongressoModality(CongressoModality modality) {
        return (root, query, cb) -> {
            if (modality == null) {
                return null;
            }
    ...
    public static Specification<JpaCongressoEntity> placeContains(String place) {
        return (root, query, cb) -> {
    ...
    public static Specification<JpaCongressoEntity> startsAtOrAfter(Date start) {
        return (root, query, cb) -> start == null ? null : cb.greaterThanOrEqualTo(root.get("startDate"), start);
    }

    public static Specification<JpaCongressoEntity> endsAtOrBefore(Date end) {
