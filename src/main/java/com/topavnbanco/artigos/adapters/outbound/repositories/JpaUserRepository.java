...
public interface JpaUserRepository extends JpaRepository<JpaUserEntity, Long> {
    ...
    @Query(value = """
        ...
        LIMIT :limit
        """, nativeQuery = true)
    List<JpaUserEntity> pickRandomEligible(
    ...
    LIMIT 1
    """, nativeQuery = true)
    Optional<JpaUserEntity> pickOneRandomEligible(
            @Param("articleId") Long articleId,
    ...
    Page<JpaUserEntity> findByLoginIgnoreCaseContaining(String login, Pageable pageable);
